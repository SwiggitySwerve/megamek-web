...
export function EnhancedSystemComponentControls() {
  ...
  const updateConfig = useCallback((updates: any) => {
    console.log('🔧 [CONFIG_UPDATE] EnhancedSystemComponentControls.updateConfig called with:', updates)
    console.log('🔧 [CONFIG_UPDATE] Current config before merge:', {
      structureType: config.structureType,
      armorType: config.armorType
    })
    const newConfig = { ...config, ...updates }
    console.log('🔧 [CONFIG_UPDATE] New config after merge:', {
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2 items-center">
                ...
                <select
                  ...
                  onChange={(e) => {
                    console.log('🔧 [STRUCTURE_DEBUG] Structure type dropdown changed to:', e.target.value)
                    console.log('🔧 [STRUCTURE_DEBUG] Current config before update:', {
                      structureType: config.structureType,
                      armorType: config.armorType
                    })
                    const newStructureType = { ...config.structureType, type: e.target.value }
                    console.log('🔧 [STRUCTURE_DEBUG] New structure type object:', newStructureType)
                    const update = { structureType: newStructureType }
                    console.log('🔧 [STRUCTURE_DEBUG] Update object being passed to updateConfig:', update)
                    updateConfig(update)
                  }}
                  className="bg-gray-700 text-white text-xs p-1 rounded border border-gray-600 focus:border-blue-500"
                >
            ...
            <div className="space-y-2">
              <div className="grid grid-cols-2 gap-2 items-center">
                ...
                <select
                  ...
                  onChange={(e) => {
                    console.log('🔧 [ARMOR_DEBUG] Armor type dropdown changed to:', e.target.value)
                    console.log('🔧 [ARMOR_DEBUG] Current config before update:', {
                      structureType: config.structureType,
                      armorType: config.armorType
                    })
                    const newArmorType = { ...config.armorType, type: e.target.value }
                    console.log('🔧 [ARMOR_DEBUG] New armor type object:', newArmorType)
                    const update = { armorType: newArmorType }
                    console.log('🔧 [ARMOR_DEBUG] Update object being passed to updateConfig:', update)
                    updateConfig(update)
                  }}
                  className="bg-gray-700 text-white text-xs p-1 rounded border border-gray-600 focus:border-blue-500"
                >
