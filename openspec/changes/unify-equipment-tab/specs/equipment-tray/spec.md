...
The Loadout Tray SHALL allow users to remove equipment via selection and button.

#### Scenario: Select and remove
- **WHEN** user clicks equipment row to select it
- **THEN** row is highlighted
- **AND** clicking "Remove" button in header removes selected item
- **AND** tray updates immediately

#### Scenario: Double-click remove
- **WHEN** user double-clicks equipment row
- **THEN** equipment is removed immediately without selection step
