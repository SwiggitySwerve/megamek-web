...
import { normalizeTechBase, getTechBaseAbbreviation } from '../../utils/techBaseUtils';
...
const UnitCompendiumList: React.FC<UnitCompendiumListProps> = ({ filters, selectedCategory }) => {
  ...
  const getTechBaseBadge = (techBaseValue: string) => {
    const normalized = normalizeTechBase(techBaseValue);
    const badgeClasses: Record<TechBase, string> = {
      [TechBase.INNER_SPHERE]: 'bg-blue-100 text-blue-800',
      [TechBase.CLAN]: 'bg-red-100 text-red-800',
      [TechBase.MIXED]: 'bg-purple-100 text-purple-800',
    };

    return (
      <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${badgeClasses[normalized]}`}>
        {getTechBaseAbbreviation(normalized)}
      </span>
    );
  };
