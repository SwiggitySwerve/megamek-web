  ...
  useEffect(() => {
    const fetchEquipment = async () => {
      ...
      try {
        ...
        const data = await response.json() as ApiListResponse<EquipmentFromApi>;
    ...
    <div className="mt-4">
      <ul className="space-y-2">
        {equipmentData.items.map((item) => (
          <li key={item.name} className="p-2 border rounded hover:bg-gray-50">
            <Link href={`/equipment/${item.name}`} legacyBehavior>
