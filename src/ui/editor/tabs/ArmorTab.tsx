...
import { ArmorType, IArmorAllocation, ILocationArmor } from '../../../types';
...
import { ARMOR_POINTS_PER_TON, calculateArmorWeight } from '../../../lib/armorCalculations';
...
export const ArmorTab: React.FC<ArmorTabProps> = ({ readOnly = false }) => {
  ...
  // Convert ArmorAllocation to IArmorAllocation format
  const armorAllocation: IArmorAllocation = Object.entries(config.armorAllocation).reduce((acc, [key, value]) => {
    acc[key] = { front: value.front, rear: value.rear };
    return acc;
  }, {} as IArmorAllocation);
  ...
  const updateConfigurationWithValidation = (newConfig: Partial<UnitConfiguration>) => {
    const currentConfig = unit.getConfiguration();
    // Merge partial update with current config
    const mergedConfig: UnitConfiguration = {
      ...currentConfig,
      ...newConfig,
    };
    
    // Validate armor tonnage if it's being updated
    if ('armorTonnage' in newConfig && typeof mergedConfig.armorTonnage === 'number') {
      mergedConfig.armorTonnage = Math.min(Math.max(mergedConfig.armorTonnage, 0), maxArmorTonnage);
    }
  ...
  const handleArmorTypeChange = (newArmorType: string) => {
    ...
    const newProgression = {
      ...currentProgression,
      armor: newTechBase as 'Inner Sphere' | 'Clan'
    };
    
    // Create the component configuration update
    const componentUpdate = { armorType: newArmorType as ArmorType };
    ...
    const finalUpdates: Partial<UnitConfiguration> = {
      ...componentUpdate,
      techProgression: newProgression as UnitConfiguration['techProgression'],
      armorTonnage: preservedTonnage, // Preserve/cap armor tonnage
      ...(isMixed && config.techBase !== 'Mixed' ? { techBase: 'Mixed' as 'Inner Sphere' | 'Clan' | 'Mixed' } : {})
    };
  ...
  const handleArmorLocationChange = (location: string, front: number, rear: number = 0) => {
    ...
    // Convert IArmorAllocation back to ArmorAllocation format
    const convertLocation = (loc: ILocationArmor | undefined): { front: number; rear: number } => {
      return loc ? { front: loc.front, rear: loc.rear ?? 0 } : { front: 0, rear: 0 };
    };
    const convertedAllocation: UnitConfiguration['armorAllocation'] = {
      HD: convertLocation(newAllocation.HD),
      CT: convertLocation(newAllocation.CT),
      LT: convertLocation(newAllocation.LT),
      RT: convertLocation(newAllocation.RT),
      LA: convertLocation(newAllocation.LA),
      RA: convertLocation(newAllocation.RA),
      LL: convertLocation(newAllocation.LL),
      RL: convertLocation(newAllocation.RL),
    };

    updateConfiguration({
      ...config,
      armorAllocation: convertedAllocation
    });
  };
