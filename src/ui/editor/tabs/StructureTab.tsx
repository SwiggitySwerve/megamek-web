...
import { calculateEnhancedMovement, getAvailableMovementEnhancements, MOVEMENT_ENHANCEMENTS } from '../../../utils/movementCalculations';
import { TechProgression } from '../../../utils/techProgression';

// Import structure and armor calculations
import { calculateStructureWeight, getStructureSlots } from '../../../types';
...
const resolveTechBase = (techBase: string): TechBase =>
  techBase === 'Clan' ? TechBase.CLAN : TechBase.INNER_SPHERE;
...
export const StructureTab: React.FC<StructureTabProps> = ({ readOnly = false }) => {
  ...
  const applyMemoryRestoration = (config: UnitConfiguration, memoryState: ComponentMemoryState): UnitConfiguration => {
    ...
    const testResult = isComponentAvailable('None', ComponentCategory.MYOMER, TechBase.INNER_SPHERE);
    ...
    Object.entries(techProgression).forEach(([subsystem, techBase]) => {
      ...
      if (savedComponent && savedComponent !== 'None' && savedComponent !== 'Standard') {
        ...
        if (configProperty) {
          (restorationUpdates as Record<string, unknown>)[configProperty] = savedComponent;
  ...
  const propertyMap: Partial<Record<keyof TechProgression, keyof UnitConfiguration>> = {
    chassis: 'structureType',
    gyro: 'gyroType', 
    engine: 'engineType',
    heatsink: 'heatSinkType',
    armor: 'armorType',
    // targeting: 'targetingType', // Not a property of UnitConfiguration
    // movement: 'movementType' // Not a property of UnitConfiguration
  ...
  const getConfigPropertyForSubsystem = (subsystem: keyof TechProgression): keyof UnitConfiguration | null => {
    // Special case for myomer/enhancements
    if (subsystem === 'myomer') {
      return 'enhancements';
    }
  ...
  const convertTechBase = (tb: 'Inner Sphere' | 'Clan'): TechBase.INNER_SPHERE | TechBase.CLAN => 
    tb === 'Clan' ? TechBase.CLAN : TechBase.INNER_SPHERE;
  const techProgression = config.techProgression ? {
    chassis: convertTechBase(config.techProgression.chassis),
    gyro: convertTechBase(config.techProgression.gyro),
    engine: convertTechBase(config.techProgression.engine),
    heatsink: convertTechBase(config.techProgression.heatsink),
    targeting: convertTechBase(config.techProgression.targeting),
    myomer: convertTechBase(config.techProgression.myomer),
    movement: convertTechBase(config.techProgression.movement),
    armor: convertTechBase(config.techProgression.armor),
  } as TechProgression : undefined;
  const filteredOptions = techProgression ? getFilteredComponentOptions(techProgression, config) : {
    structure: [],
    gyro: [],
    engine: [],
    heatSink: [],
  ...
  const movementConfig: UnitConfiguration = {
    ...config,
  ...
  const updateConfig = (updates: Partial<CanonicalUnitConfiguration>) => {
    ...
    if ('tonnage' in updates || 'walkMP' in updates || 'enhancements' in updates) {
      ...
      const movementConfig: UnitConfiguration = {
        ...
        jumpMP: newConfig.jumpMP ?? config.jumpMP,
