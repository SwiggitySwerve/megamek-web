...
const SPECIAL_COMPONENT_KEYWORDS = [
  'endo steel',
  'composite structure',
...
const isSpecialComponentName = (name: string): boolean => {
  const normalized = name.toLowerCase()
  return SPECIAL_COMPONENT_KEYWORDS.some(keyword => normalized.includes(keyword))
}
...
export const DraggableEquipmentItem: React.FC<DraggableEquipmentItemProps> = ({
  ...
  if (isSpecialComponentName(equipment.name) && !equipment.name.includes('Heat Sink')) {
    critSlots = 1;
  }
  ...
  const dragItem: DraggedEquipment = {
    ...
    category: equipment.type as EquipmentCategory,
