  ...
  const filteredEquipment = useMemo(() => {
    const filtered = SAMPLE_EQUIPMENT.filter(equipment => {
      ...
      if (!showUnavailable) {
        ...
        const techId = String(equipment.id).replace(/_/g, '_');
    ...
    <div className="equipment-database bg-white rounded-lg border border-gray-200 p-4">
      ...
      <div className="equipment-list max-h-96 overflow-y-auto space-y-2">
        {filteredEquipment.map(equipment => {
          ...
          const techId = equipmentId.replace(/_/g, '_');
          const isAvailable = isEquipmentAvailable(techId, currentYear, faction);
          ...
          return (
            <div
              ...
              onClick={() => onEquipmentSelect(
                selectedEquipment === equipmentId ? null : equipmentId
              )}
            >
              {/* Equipment Header */}
              ...
              {selectedEquipment === equipment.id && (
                <div className="mt-3 pt-3 border-t border-gray-200">
                  <div className="text-xs text-gray-600 space-y-1">
                    ...
                    {(() => {
                      const specials = equipment.data?.specials;
                      const specialText = Array.isArray(specials)
                        ? specials.join(', ')
                        : typeof specials === 'string'
                          ? specials
                          : specials
                      ...
                      return specialText ? (
                        <div>
                          <strong>Special Rules:</strong> {specialText}
                        </div>
                      ) : null;
                    })()}
                  </div>
