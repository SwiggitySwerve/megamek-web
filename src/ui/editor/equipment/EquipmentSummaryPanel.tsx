...
import { EditableUnit, ValidationError, EquipmentPlacement } from '../../../types';
...
const resolveEquipmentCategory = (equipment: EquipmentPlacement['equipment']): string => {
  if (equipment && typeof equipment === 'object' && 'category' in equipment && typeof equipment.category === 'string') {
    return equipment.category || 'Other'
  }
  if (equipment && typeof equipment === 'object' && 'type' in equipment && typeof equipment.type === 'string') {
    return equipment.type || 'Other'
  }
  return 'Other'
}
...
const EquipmentSummaryPanel: React.FC<EquipmentSummaryPanelProps> = ({
  ...
  const equipmentByCategory = (unit.equipmentPlacements || []).reduce((acc, placement) => {
    const category = resolveEquipmentCategory(placement.equipment);
