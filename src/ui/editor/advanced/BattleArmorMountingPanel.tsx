...
const BattleArmorMountingPanel: React.FC<BattleArmorMountingPanelProps> = ({
  ...
  interface EditableUnitWithBattleArmor extends EditableUnit {
    mountedBattleArmor?: MountedBattleArmor[];
    hasOmniMounts?: boolean;
  }

  const getMountedBattleArmor = (editable: EditableUnit): MountedBattleArmor[] => {
    const unitWithBA = editable as EditableUnitWithBattleArmor;
    return unitWithBA.mountedBattleArmor ?? [];
  };

  const [mountedBA, setMountedBA] = useState<MountedBattleArmor[]>(getMountedBattleArmor(unit));

  useEffect(() => {
    setMountedBA(getMountedBattleArmor(unit));
  }, [unit]);
  ...
  const getMountingLocations = useCallback(() => {
    ...
    const isQuad = chassis.includes('quad') || model.includes('quad');
  ...
  const unitWithBA = unit as EditableUnitWithBattleArmor;
  const hasOmniMounts = Boolean(unitWithBA.hasOmniMounts);
  ...
  const handleMountNewBA = useCallback((baType: { name: string, squad: number }) => {
    ...
    const newBA: MountedBattleArmor = {
      ...
      isOmniMount: hasOmniMounts,
    };

    handleMountBA(newBA);
  }, [mountedBA, hasOmniMounts, readOnly, getMountingLocations, canMountAtLocation, handleMountBA]);
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-xs">
                    {baAtLocation.map((ba) => (
                      <div key={ba.name} className="flex items-center space-x-2 mb-1">
