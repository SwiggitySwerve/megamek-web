...
const QuadVeeModeSwitcher: React.FC<QuadVeeModeSwitcherProps> = ({
  ...
  interface EditableUnitWithQuadVee extends EditableUnit {
    quadVeeConfiguration?: {
      currentMode?: QuadVeeMode;
      conversionEquipmentDamaged?: boolean;
      lastTransformation?: number;
    };
  }

  const unitWithQuadVee = unit as EditableUnitWithQuadVee;
  const quadVeeConfig = unitWithQuadVee.quadVeeConfiguration ?? {
    currentMode: undefined,
    conversionEquipmentDamaged: false,
    lastTransformation: undefined
  };

  const [currentMode, setCurrentMode] = useState<QuadVeeMode>(
    quadVeeConfig.currentMode || 'Mech'
  );
    ...
    setTimeout(() => {
      ...
      const updatedUnit = {
        ...
        quadVeeConfiguration: {
          ...
          conversionEquipmentDamaged: quadVeeConfig.conversionEquipmentDamaged || false,
        },
      } as EditableUnit;
  ...
  const canTransform = useCallback((targetMode: QuadVeeMode): boolean => {
    ...
    if (quadVeeConfig.conversionEquipmentDamaged) {
      return false;
    }
  ...
  return (
    <div className="quadvee-mode-switcher bg-white rounded-lg border border-gray-200 p-4">
      ...
      <span className={`font-medium ${
            quadVeeConfig.conversionEquipmentDamaged ? 'text-red-600' : 'text-green-600'
          }`}>
            {quadVeeConfig.conversionEquipmentDamaged ? 'Damaged' : 'Functional'}
          </span>
        </div>
      </div>
      ...
      {/* Warnings */}
      {quadVeeConfig.conversionEquipmentDamaged && (
