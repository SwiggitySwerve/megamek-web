...
import { EditableUnit, ValidationError } from '../../../types';
...
const ArmorValidationDisplay: React.FC<ArmorValidationDisplayProps> = ({
  ...
  const validations = useMemo((): ArmorValidation[] => {
    ...
    if (armorType.type === 'Stealth') {
      const hasGuardianECM = unit.equipmentPlacements?.some((placement) =>
        placement.equipment.name?.includes('Guardian ECM')
      );
      if (!hasGuardianECM) {
        results.push({
          type: 'error',
