...
import { EditableUnit, MECH_LOCATIONS, IArmorDef } from '../../../types';
import { ArmorLocationControl } from './ArmorLocationControl';
import { calculateMaxArmorPoints } from '../../../lib/armorCalculations';
...
export interface InteractiveMechArmorDiagramProps {
  ...
  onArmorTypeChange?: (location: string, armorType: IArmorDef) => void;
...
export const InteractiveMechArmorDiagram: React.FC<InteractiveMechArmorDiagramProps> = ({
  ...
  const getLocationArmor = (location: string) => {
    ...
    const armorDefinition: IArmorDef | undefined = undefined;
    ...
    switch (location) {
      case MECH_LOCATIONS.HEAD:
        return { front: alloc.head, rear: 0, maxArmor: 9, type: armorDefinition };
      case MECH_LOCATIONS.CENTER_TORSO:
        return { front: alloc.centerTorso, rear: alloc.centerTorsoRear, maxArmor: calculateMaxArmorPoints(unit.tonnage || 0) * 2 * 0.4, type: armorDefinition }; 
      case MECH_LOCATIONS.LEFT_TORSO:
        return { front: alloc.leftTorso, rear: alloc.leftTorsoRear, maxArmor: calculateMaxArmorPoints(unit.tonnage || 0) * 2 * 0.3, type: armorDefinition };
      case MECH_LOCATIONS.RIGHT_TORSO:
        return { front: alloc.rightTorso, rear: alloc.rightTorsoRear, maxArmor: calculateMaxArmorPoints(unit.tonnage || 0) * 2 * 0.3, type: armorDefinition };
      case MECH_LOCATIONS.LEFT_ARM:
        return { front: alloc.leftArm, rear: 0, maxArmor: calculateMaxArmorPoints(unit.tonnage || 0) * 2 * 0.25, type: armorDefinition };
      case MECH_LOCATIONS.RIGHT_ARM:
        return { front: alloc.rightArm, rear: 0, maxArmor: calculateMaxArmorPoints(unit.tonnage || 0) * 2 * 0.25, type: armorDefinition };
      case MECH_LOCATIONS.LEFT_LEG:
        return { front: alloc.leftLeg, rear: 0, maxArmor: calculateMaxArmorPoints(unit.tonnage || 0) * 2 * 0.25, type: armorDefinition };
      case MECH_LOCATIONS.RIGHT_LEG:
        return { front: alloc.rightLeg, rear: 0, maxArmor: calculateMaxArmorPoints(unit.tonnage || 0) * 2 * 0.25, type: armorDefinition };
      default:
