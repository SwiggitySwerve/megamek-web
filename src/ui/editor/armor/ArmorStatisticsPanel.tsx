...
import { ARMOR_SPECIFICATIONS } from '../../../lib/armorCalculations';
...
export const ArmorStatisticsPanel: React.FC<ArmorStatisticsProps> = ({
  ...
  const currentArmorType =
    ARMOR_TYPE_OPTIONS.find(type => type.name === unit.armor?.type) || ARMOR_TYPE_OPTIONS[0];
  ...
  const calculateArmorStats = () => {
    ...
    const structureMax = (unit as EditableUnit & { structure?: { maxPoints?: Record<string, number> } }).structure?.maxPoints;

    if (unit.armor?.allocation && structureMax) {
      const alloc = unit.armor.allocation;
      const maxPoints = structureMax;
