...
import { slotEquipmentFromFull } from './utils/equipmentNormalization';
// Icon components
...
export const EquipmentManagementComponent: React.FC<EquipmentManagementComponentProps> = ({
  ...
  const handleEquipmentDrop = useCallback((item: DragItem, location: string, slotIndex?: number) => {
    ...
    const slotEquipment = slotEquipmentFromFull(item.equipment);
    ...
    if (result.placements.length > 0) {
      ...
      result.placements.forEach(placement => {
        if (placement.success) {
          const slotEquipment = slotEquipmentFromFull(placement.equipment);
