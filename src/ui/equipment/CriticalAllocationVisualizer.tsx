...
import { EditableUnit, CriticalSlotAssignment, SlotEquipment, FullEquipment } from '../../types';

const isFullEquipment = (equipment: SlotEquipment): equipment is FullEquipment => {
  return 'data' in equipment;
};

interface CriticalAllocationVisualizerProps {
  unit: EditableUnit;
  onSlotClick?: (location: string, slotIndex: number) => void;
  onEquipmentClick?: (equipment: SlotEquipment, location: string) => void;
...
export const CriticalAllocationVisualizer: React.FC<CriticalAllocationVisualizerProps> = ({
  ...
  // Get equipment color
const getEquipmentColor = useCallback((equipment?: SlotEquipment): string => {
    ...
    if ('type' in equipment) {
      ...
      } else if (equipment.type && typeof equipment.type === 'object' && 'toString' in equipment.type) {
        equipmentType = equipment.type.toString();
      }
    }

    if (equipmentType.includes('Weapon')) {
      if (isFullEquipment(equipment)) {
        const weaponType = equipment.data?.weapon_type;
        if (weaponType === 'Energy') return EQUIPMENT_COLORS['Energy Weapon'];
        if (weaponType === 'Ballistic') return EQUIPMENT_COLORS['Ballistic Weapon'];
        if (weaponType === 'Missile') return EQUIPMENT_COLORS['Missile Weapon'];
      }
    }

    return EQUIPMENT_COLORS[equipmentType] || EQUIPMENT_COLORS['Equipment'];
  }, []);
