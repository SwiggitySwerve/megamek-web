...
import { FullEquipment, EditableUnit, CriticalSlotAssignment, SlotEquipment, TechBase } from '../../../types';
...
import { ComponentConfiguration as CoreComponentConfiguration } from '../../../types';
import { EngineType, GyroType } from '../../../types';
import { fullEquipmentFromSlotEquipment, normalizeFullEquipment, slotEquipmentFromFull } from '../utils/equipmentNormalization';
...
export function useEquipmentPlacement(): UseEquipmentPlacementReturn {
  ...
  const autoPlace = useCallback((
    ...
    sortedEquipment.forEach(item => {
      const normalizedEquipment = normalizeFullEquipment(item);
      const suggestions = suggestPlacement(normalizedEquipment, workingUnit);
      
      if (suggestions.length > 0) {
        const bestSuggestion = suggestions[0];
        const slotEquipment = slotEquipmentFromFull(normalizedEquipment);
  ...
  const optimizePlacements = useCallback((
    ...
    (unit.criticalSlots || []).forEach(slot => {
      if (slot.equipment && !slot.isFixed) {
        const normalizedEquipment = fullEquipmentFromSlotEquipment(slot.equipment as SlotEquipment);
        ...
        if (!locationEquipment.find(eq => eq.id === normalizedEquipment.id)) {
          locationEquipment.push(normalizedEquipment);
          currentPlacements.push({ equipment: normalizedEquipment, location: slot.location });
        }
      }
    });
        ...
        if (improvement > 10) { // Only suggest if significant improvement
          ...
          suggestions[0].slots.forEach(slotIndex => {
            ...
            if (existingSlotIndex >= 0) {
              criticalSlots[existingSlotIndex] = {
                ...criticalSlots[existingSlotIndex],
                equipment: slotEquipmentFromFull(equipment),
                isEmpty: false,
              };
            }
            workingUnit.criticalSlots = criticalSlots;
          ...
          workingUnit.criticalSlots = criticalSlots.map(slot => {
            if (slot.location === currentLocation && 
              ...
              return { ...slot, equipment: slotEquipmentFromFull(equipment), isEmpty: false };
            }
            return slot;
          });
        }
