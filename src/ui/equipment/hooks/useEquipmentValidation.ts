...
import { EditableUnit, FullEquipment, SlotEquipment, TechBase } from '../../../types';
import { normalizeTechBase } from '../../../utils/techBaseUtils';
...
export function useEquipmentValidation(): UseEquipmentValidationReturn {
  ...
  const extractHeatValue = (equipment: SlotEquipment): number => {
    if ('heatGeneration' in equipment && typeof equipment.heatGeneration === 'number') {
      return equipment.heatGeneration;
    }
    if ('heat' in equipment && typeof equipment.heat === 'number') {
  ...
  const getTotalHeatGeneration = useCallback((unit: EditableUnit): number => {
    return (unit.equipmentPlacements || []).reduce((total, placement) => {
      const heat = extractHeatValue(placement.equipment);
      return total + heat;
    }, 0);
  }, []);
  ...
  const checkTechBaseCompatibility = useCallback((
    ...
    if (!equipment.tech_base || !unit.tech_base) {
      return true;
    }

    const unitTechBase = normalizeTechBase(unit.tech_base);
    const equipmentTechBase = normalizeTechBase(equipment.tech_base);

    if (unitTechBase === TechBase.MIXED || equipmentTechBase === TechBase.MIXED) {
      return true;
    }

    return unitTechBase === equipmentTechBase;
  }, [validationOptions]);
