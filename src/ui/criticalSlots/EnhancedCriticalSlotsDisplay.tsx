...
export function EnhancedCriticalSlotsDisplay() {
  ...
  const prevUnallocatedRef = useRef<EquipmentAllocation[]>([])
  const prevSelectedRef = useRef<string | null>(null)
  
  // Type guard to validate CriticalSlotsToolbarState structure
  const isValidToolbarState = (value: unknown): value is CriticalSlotsToolbarState => {
    if (!value || typeof value !== 'object') {
      return false;
    }
    const obj = value as Record<string, unknown>;
    return (
      typeof obj.autoFillUnhittables === 'boolean' &&
      typeof obj.autoCompact === 'boolean' &&
      typeof obj.autoSort === 'boolean'
    );
  };
  ...
  const getToolbarState = (): CriticalSlotsToolbarState => {
    try {
      ...
      if (saved) {
        ...
        // Validate the parsed object matches CriticalSlotsToolbarState structure
        if (isValidToolbarState(parsed)) {
          return parsed;
        }
      }
    } catch (error) {
