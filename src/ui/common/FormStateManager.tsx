...
export interface UseFormStateReturn<T extends Record<string, unknown>> {
  ...
  getFieldProps: <K extends keyof T>(field: K) => {
    ...
    isDirty: boolean
    isTouched: boolean
    validation?: (value: T[K]) => ValidationResult
  }
}
  ...
  switch (action.type) {
    ...
    case 'SET_FIELD_DIRTY': {
      ...
      return {
        ...
        isDirty: dirty || Object.values(state.fields).some((f: FormField<unknown>) => f.isDirty)
      }
    }
    ...
    case 'VALIDATE_ALL': {
      ...
      for (const [key, field] of Object.entries(fields)) {
        ...
        if (typedField.validation) {
          const result = typedField.validation(typedField.value)
          if (result.error) {
...
export function useFormState<T extends Record<string, unknown>>(
  ...
  const getValues = useCallback((): T => {
    ...
    for (const [key, field] of Object.entries(state.fields)) {
      const typedField = field as FormField<T[keyof T]>
      values[key as keyof T] = typedField.value as T[keyof T]
    }
    return values as T
  }, [state.fields])
...
export interface FormFieldProps<T extends Record<string, unknown>, K extends keyof T> {
