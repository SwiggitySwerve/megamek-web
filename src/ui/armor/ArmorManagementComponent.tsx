...
import { autoAllocateArmor, convertToIArmorAllocation } from '../../lib/armorAllocation';
...
// Helper to convert ArmorProfile to IArmorDef
const convertArmorTypeToDef = (type: ArmorProfile): IArmorDef => ({
...
const ArmorManagementComponent: React.FC<ArmorManagementComponentProps> = ({
  ...
  const unitTonnage: number = (() => {
    if ('tonnage' in unit && typeof unit.tonnage === 'number') {
      return unit.tonnage;
    }
    if ('mass' in unit && typeof unit.mass === 'number') {
      return unit.mass;
    }
    return 50;
  })();
  ...
  const handleApplyDistribution = useCallback((distribution: Record<string, { front: number; rear?: number }>) => {
    if (readOnly || !onUnitChange) return;
