...
import { EquipmentObject, EquipmentAllocation, LocationSlotConfiguration, TechBase } from '../../types'
import { EngineType, GyroType, SystemComponentRules } from './SystemComponentRules'
import { ComponentConfiguration } from '../../types/ComponentConfiguration'
import { UnitConfiguration as UnitConfigurationBase, ArmorAllocation as ArmorAllocationBase } from './UnitCriticalManagerTypes'

// ===== CORE ORCHESTRATOR TYPES =====

/**
 * Unit Configuration Interface
 * Extends the base UnitConfiguration from UnitCriticalManagerTypes
 */
export type UnitConfiguration = UnitConfigurationBase

/**
 * Armor Allocation Interface
 * Uses the base type from UnitCriticalManagerTypes
 */
export type ArmorAllocation = ArmorAllocationBase
...
export interface OperationParameters {
  ...
  [key: string]: string | number | boolean | EquipmentObject | Partial<UnitConfiguration> | undefined
}
...
/**
 * Orchestrator Result Data Types
 */
export type OrchestratorResultData = 
  | AllocationResult
  | WeightAnalysis
  | UnitValidationResult
  | ComplianceReport
  | UnitState
  | string // For serialized state
  | null
...
export interface UnitOrchestrator {
  ...
  deserializeState(state: UnitState | string): Promise<OrchestratorResult>
...
export interface ISystemComponentService {
  getEngineSlots(engineType: EngineType, engineRating: number): number[]
  getGyroSlots(gyroType: GyroType): number[]
  validateSystemComponents(config: UnitConfiguration): ValidationResult
}

export interface IWeightBalanceService {
  calculateTotalWeight(config: UnitConfiguration, equipment: EquipmentAllocation[]): number
  calculateRemainingTonnage(config: UnitConfiguration, equipment: EquipmentAllocation[]): number
  isOverweight(config: UnitConfiguration, equipment: EquipmentAllocation[]): boolean
}

export interface IEquipmentAllocationService {
  allocateEquipment(equipment: EquipmentObject, location: string, startSlot: number): Promise<boolean>
  deallocateEquipment(equipmentGroupId: string): Promise<boolean>
  findAvailableSlots(location: string, requiredSlots: number): number[]
}

export interface IConstructionRulesValidator {
  validateConfiguration(config: UnitConfiguration): ValidationResult
  validateEquipmentPlacement(equipment: EquipmentObject, location: string): ValidationResult
  validateWeightLimits(config: UnitConfiguration, totalWeight: number): ValidationResult
}
...
export interface UnitOrchestratorDependencies {
  ...
  constructionRulesValidator?: IConstructionRulesValidator
  performanceMonitor?: PerformanceMonitor
  stateNotifier?: StateChangeNotifier
  commandRegistry?: CommandRegistry
  commandHistory?: CommandHistory
  cache?: Cache<OrchestratorResultData>
}
 ...
 */
export const isUnitConfiguration = (obj: unknown): obj is UnitConfiguration => {
  return typeof obj === 'object' && obj !== null && 
         'tonnage' in obj && typeof (obj as { tonnage: unknown }).tonnage === 'number' &&
         'engineRating' in obj && typeof (obj as { engineRating: unknown }).engineRating === 'number'
}

export const isOrchestratorResult = (obj: unknown): obj is OrchestratorResult => {
  return typeof obj === 'object' && obj !== null && 
         'success' in obj && typeof (obj as { success: unknown }).success === 'boolean'
}

export const isValidationResult = (obj: unknown): obj is ValidationResult => {
  return typeof obj === 'object' && obj !== null &&
         'isValid' in obj && typeof (obj as { isValid: unknown }).isValid === 'boolean' &&
         'errors' in obj && Array.isArray((obj as { errors: unknown }).errors)
}

export const isWeightAnalysis = (obj: unknown): obj is WeightAnalysis => {
  return typeof obj === 'object' && obj !== null &&
         'totalWeight' in obj && typeof (obj as { totalWeight: unknown }).totalWeight === 'number' &&
         'maximumWeight' in obj && typeof (obj as { maximumWeight: unknown }).maximumWeight === 'number'
}
