...
export interface SlotReport {
  ...
  specialComponents: {
    endoSteel: { required: number; allocated: Record<string, unknown>; conflicts: string[] };
    ferroFibrous: { required: number; allocated: Record<string, unknown>; conflicts: string[] };
    altri: { required: number; allocated: Record<string, unknown>; conflicts: string[] };
  };
  conflicts: Array<{
    type: string;
    location: string;
...
export class SlotValidationManager {
   ...
   */
  validateSlotAllocation(config: UnitConfiguration, equipment: EquipmentObject[]): ValidationResult {
    const errors: ValidationError[] = [];
    const warnings: ValidationWarning[] = [];
    const locationStatus: ValidationResult['locationStatus'] = {};
   ...
   */
  analyzeSlotEfficiency(config: UnitConfiguration, equipment: EquipmentObject[]): EfficiencyAnalysis {
    const locationEfficiency: EfficiencyAnalysis['locationEfficiency'] = {};
   ...
   */
  generateSlotReport(config: UnitConfiguration, equipment: EquipmentObject[]): SlotReport {
    ...
    const locationBreakdown: SlotReport['locationBreakdown'] = {};
    Object.entries(this.STANDARD_SLOT_COUNTS).forEach(([location, capacity]) => {
   ...
   */
  findAvailableSlots(config: UnitConfiguration, equipment: EquipmentObject[], requiredSlots: number): AvailableSlotLocation[] {
   ...
   */
  private getUsedSlotsInLocation(equipment: EquipmentObject[], location: string): number {
    return equipment.reduce((total, item) => {
      // EquipmentObject doesn't have location property, this method may need refactoring
      // For now, we'll check if it's an EquipmentAllocation-like object
      const itemWithLocation = item as EquipmentObject & { location?: string };
      if (itemWithLocation.location === location) {
   ...
   */
  private getConflictsInLocation(equipment: EquipmentObject[], location: string): string[] {
    const conflicts: string[] = [];
    const usedSlots = new Set<number>();

    equipment.forEach(item => {
      const itemWithLocation = item as EquipmentObject & { location?: string; slots?: number[] };
      if (itemWithLocation.location === location && itemWithLocation.slots) {
        itemWithLocation.slots.forEach((slot: number) => {
   ...
   * Validate BattleTech rules
   */
  private validateBattleTechRules(config: UnitConfiguration, equipment: EquipmentObject[], errors: ValidationError[], warnings: ValidationWarning[]): void {
   ...
   */
  private generateLocationSuggestions(location: string, locationData: EfficiencyAnalysis['locationEfficiency'][string]): string[] {
    const suggestions: string[] = [];
   ...
   */
  private calculateOptimizationPotential(locationEfficiency: EfficiencyAnalysis['locationEfficiency']): number {
    let potential = 0;
    let count = 0;

    Object.values(locationEfficiency).forEach((data) => {
   ...
   */
  private generateEfficiencyRecommendations(locationEfficiency: EfficiencyAnalysis['locationEfficiency'], overallEfficiency: number): string[] {
    ...
    Object.entries(locationEfficiency).forEach(([location, data]) => {
   ...
   */
  private calculateOverallEfficiency(config: UnitConfiguration, equipment: EquipmentObject[]): string {
    const totalSlots = Object.values(this.STANDARD_SLOT_COUNTS).reduce((sum, count) => sum + count, 0);
    const usedSlots = equipment.reduce((total, item) => total + (item.requiredSlots ?? 1), 0);
    const utilization = totalSlots > 0 ? (usedSlots / totalSlots) * 100 : 0;
   ...
   */
  private getEquipmentInLocation(equipment: EquipmentObject[], location: string): string[] {
    return equipment
      .filter(item => {
        const itemWithLocation = item as EquipmentObject & { location?: string };
        return itemWithLocation.location === location;
      })
   ...
   */
  private getAllocatedSpecialComponents(config: UnitConfiguration): SlotReport['specialComponents'] {
   ...
   */
  private detectAllConflicts(config: UnitConfiguration, equipment: EquipmentObject[]): SlotReport['conflicts'] {
   ...
   */
  private generateReportRecommendations(summary: SlotReport['summary'], locationBreakdown: SlotReport['locationBreakdown'], conflicts: SlotReport['conflicts']): string[] {
    const recommendations: string[] = [];
