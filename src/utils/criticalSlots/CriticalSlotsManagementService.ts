...
import { isError } from '../../types/adapters'
...
export class CriticalSlotsManagementService {
  ...
  static fillUnhittables(unitManager: UnitCriticalManager): CriticalSlotsOperationResult {
    try {
      ...
      console.error('[CriticalSlotsManagementService] Error during enhanced fill operation:', errorMessage)
      return {
        success: false,
        message: `Error during auto-allocation: ${errorMessage}`,
        slotsModified: 0
      }
    }
  }
  ...
  static compact(unitManager: UnitCriticalManager): CriticalSlotsOperationResult {
    try {
      ...
      const errorMessage = isError(error) ? error.message : String(error);
      console.error('Error compacting slots:', errorMessage)
      return {
        success: false,
        message: `Error compacting: ${errorMessage}`,
        slotsModified: 0
      }
    }
  }
  ...
  static sort(unitManager: UnitCriticalManager): CriticalSlotsOperationResult {
    try {
      ...
      const errorMessage = isError(error) ? error.message : String(error);
      console.error('Error sorting slots:', errorMessage)
      return {
        success: false,
        message: `Error sorting: ${errorMessage}`,
        slotsModified: 0
      }
    }
  }
  ...
  ): { success: boolean; displacedEquipment: EquipmentObject[]; message: string } {
    try {
      console.log(`[CriticalSlotsManagementService] Smart slot update for ${componentType}`)
      
      const displacedEquipment: EquipmentObject[] = []
      const sections = this.getAllSections(unitManager)
      ...
      const errorMessage = isError(error) ? error.message : String(error);
      console.error('[CriticalSlotsManagementService] Error during smart slot update:', errorMessage)
      return {
        success: false,
        displacedEquipment: [],
        message: `Error during smart update: ${errorMessage}`
      }
    }
  }
  ...
  static validateComponentCompatibility(
    engineType: string,
    gyroType: string,
    techProgression: Record<string, string>
  ...
  static reset(unitManager: UnitCriticalManager): CriticalSlotsOperationResult {
    try {
      ...
      const errorMessage = isError(error) ? error.message : String(error);
      console.error('Error resetting slots:', errorMessage)
      return {
        success: false,
        message: `Error resetting: ${errorMessage}`,
        slotsModified: 0
      }
    }
  }
  ...
  private static getSpecialComponentsToAllocate(config: UnitConfiguration): Array<{type: string, count: number}> {
   ...
   */
  private static getUnhittableEquipment(unallocatedEquipment: EquipmentObject[]): EquipmentObject[] {
  ...
  private static updateSectionSlots(section: CriticalSection, newSlots: CriticalSlot[], unitManager: UnitCriticalManager): void {
  ...
  interface SlotConflict {
    index: number;
    oldContent: CriticalSlot['content'];
    newContent: CriticalSlot['content'];
    type: string;
  }

  private static findSlotConflicts(oldSlots: CriticalSlot[], newSlots: CriticalSlot[]): SlotConflict[] {
  ...
  private static resolveSlotConflict(conflict: SlotConflict, unitManager: UnitCriticalManager): { displacedEquipment: EquipmentObject[] } {
    const displacedEquipment: EquipmentObject[] = []
