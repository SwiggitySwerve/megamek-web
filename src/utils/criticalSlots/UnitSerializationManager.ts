...
import { CompleteUnitState, SerializedEquipment, SerializedSlotAllocations, StateValidationResult, UnitConfiguration } from './UnitCriticalManagerTypes';
import { EquipmentAllocation } from '../../types';
import { CriticalSection } from './CriticalSection';

/**
 * Interface for manager objects that can be serialized
 */
export interface ISerializableUnitManager {
  unallocatedEquipment: EquipmentAllocation[];
  sections: Map<string, CriticalSection>;
  configuration: UnitConfiguration;
  clearAllEquipment(): void;
  rebuildSystemComponents(skipSpecialComponents?: boolean): void;
...
export class UnitSerializationManager {
   ...
   */
  serializeCompleteState(manager: ISerializableUnitManager): CompleteUnitState {
    ...
    manager.sections.forEach((section: CriticalSection, location: string) => {
   ...
   */
  deserializeCompleteState(manager: ISerializableUnitManager, state: CompleteUnitState): boolean {
    // manager: UnitCriticalManager
   ...
   */
  restoreAllocatedEquipment(manager: ISerializableUnitManager, allocations: SerializedSlotAllocations): void {
   ...
   */
  restoreUnallocatedEquipment(manager: ISerializableUnitManager, unallocatedEquipment: SerializedEquipment[]): void {
   ...
   */
  addToUnallocatedFromSerialized(manager: ISerializableUnitManager, serializedEquipment: SerializedEquipment): void {
    const allocation = this.deserializeEquipment(serializedEquipment);
    if (allocation) {
