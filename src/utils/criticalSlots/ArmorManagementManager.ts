...
import { getArmorSpecification } from '../../lib/armorCalculations'
import { getInternalStructurePoints, InternalStructurePoints } from '../../lib/internalStructureTable'

/**
 * Armor waste analysis result
 */
export interface ArmorWasteAnalysis {
  maxArmorPoints: number
  allocatedPoints: number
  wastedPoints: number
  wastedTonnage: number
  hasWaste: boolean
}
...
export class ArmorManagementManager {
   ...
   */
  getArmorWasteAnalysis(): ArmorWasteAnalysis {
    const maxArmorPoints = this.getMaxArmorPoints()
    const allocatedPoints = this.getAllocatedArmorPoints()
