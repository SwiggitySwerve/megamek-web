...
import { EngineType, StructureType, HeatSinkType, ComponentConfiguration, TechBase } from '../types';
import { UnitConfiguration } from './criticalSlots/UnitCriticalManagerTypes';
import { COMPONENT_DATABASE } from './componentDatabase';
import { ComponentSpec } from '../types/ComponentDatabase';
...
export function getAvailableArmorTypes(config: Partial<UnitConfiguration>, overrideTechBase?: string): ComponentConfiguration[] {
  ...
  let armorComponents: ComponentSpec[] = [];
  if (techBase === 'Mixed') {
  ...
  const availableComponents = armorComponents.filter((component: ComponentSpec) => {
    return component.introductionYear <= introductionYear;
  });

  return availableComponents.map((component: ComponentSpec) =>
    createComponentConfig(component.name, (component.techBase as TechBase) || techBase)
  );
}
...
export function getAvailableStructureTypes(config: Partial<UnitConfiguration>, overrideTechBase?: TechBase): ComponentConfiguration[] {
  ...
  // Filter by introduction year and rules level
  const availableComponents = structureComponents.filter((component: ComponentSpec) => {
    return component.introductionYear <= introductionYear;
  });
...
export function getAvailableEngineTypes(config: Partial<UnitConfiguration>, overrideTechBase?: TechBase): ComponentConfiguration[] {
  ...
  // Filter by introduction year and rules level
  const availableComponents = engineComponents.filter((component: ComponentSpec) => {
    return component.introductionYear <= introductionYear;
  });
...
export function getAvailableGyroTypes(config: Partial<UnitConfiguration>, overrideTechBase?: TechBase): ComponentConfiguration[] {
  ...
  const availableComponents = gyroComponents.filter((component: ComponentSpec) => {
    return component.introductionYear <= introductionYear;
  });
...
export function getAvailableHeatSinkTypes(config: Partial<UnitConfiguration>, overrideTechBase?: TechBase): ComponentConfiguration[] {
  ...
  // Filter by introduction year and rules level
  const availableComponents = heatSinkComponents.filter((component: ComponentSpec) => {
    return component.introductionYear <= introductionYear;
  });
